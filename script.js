...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ------------------------------------------------------------------------
     GLOBAL ACTIVITY LOGGER ENGINE (SAVED TO LOCALSTORAGE)
     ------------------------------------------------------------------------ */
  function logActivity(title, detail) {
    try {
      const logs = JSON.parse(localStorage.getItem('sweetheart_activity_log') || '[]');
      const entry = {
        title: title,
        detail: detail,
      ...
      logs.unshift(entry);
      localStorage.setItem('sweetheart_activity_log', JSON.stringify(logs));
    } catch (e) {
  ...
  function showTodPrompt(mode) {
    ...
    logActivity(`Truth & Dare (${mode}) 🍾`, todPromptText.textContent);
  }
  ...
  if (btnGame4Submit) {
    btnGame4Submit.addEventListener('click', () => {
      ...
      logActivity(`Game 4 Q${game4CurrentIndex + 1} Answer 💬`, `Q: ${qObj.q} -> Ans: ${userAns}`);
  ...
  /* ------------------------------------------------------------------------
     12. SECRET ACTIVITY LOCKER VAULT (PASSCODE: 6342)
     ------------------------------------------------------------------------ */
  const SECRET_ACT_PIN = '6342';
  let enteredActPin = '';

  const btnOpenActivityLocker = document.getElementById('btn-open-activity-locker');
  const activityPinModal = document.getElementById('activity-pin-modal');
  const actBtnPinClose = document.getElementById('act-btn-pin-close');
  const actPinErrorMsg = document.getElementById('act-pin-error-msg');
  const actPinDots = document.querySelectorAll('.act-pin-dot');
  const actKeyBtns = document.querySelectorAll('.act-key-btn');

  const activityVaultModal = document.getElementById('activity-vault-modal');
  const vaultClose = document.getElementById('vault-close');
  const vaultLogContainer = document.getElementById('vault-log-container');
  const btnCopyLog = document.getElementById('btn-copy-log');
  const btnClearLog = document.getElementById('btn-clear-log');

  if (btnOpenActivityLocker) {
    btnOpenActivityLocker.addEventListener('click', () => {
      resetActPinState();
      if (activityPinModal) {
        activityPinModal.classList.add('active');
        activityPinModal.setAttribute('aria-hidden', 'false');
      }
    });
  }

  function resetActPinState() {
    enteredActPin = '';
    updateActPinDots();
    if (actPinErrorMsg) actPinErrorMsg.classList.add('hidden');
  }

  function updateActPinDots() {
    actPinDots.forEach((dot, index) => {
      if (index < enteredActPin.length) {
        dot.classList.add('filled');
      } else {
  ...
  function handleActKeypress(val) {
    if (val === 'clear') {
      if (enteredActPin.length > 0) {
        enteredActPin = enteredActPin.slice(0, -1);
        updateActPinDots();
      }
      return;
    }
  ...
  function verifyActPin() {
    if (enteredActPin === SECRET_ACT_PIN) {
      closeActPinModal();
      openActivityVault();
    } else {
      if (actPinErrorMsg) actPinErrorMsg.classList.remove('hidden');
      setTimeout(resetActPinState, 900);
    }
  }

  function closeActPinModal() {
    if (activityPinModal) {
      activityPinModal.classList.remove('active');
      activityPinModal.setAttribute('aria-hidden', 'true');
    }
  }

  actKeyBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const val = btn.getAttribute('data-val') || 'close';
      handleActKeypress(val);
    });
  });

  if (actBtnPinClose) actBtnPinClose.addEventListener('click', closeActPinModal);

  function openActivityVault() {
    renderVaultLogs();
    if (activityVaultModal) {
      activityVaultModal.classList.add('active');
      activityVaultModal.setAttribute('aria-hidden', 'false');
    }
  }

  function renderVaultLogs() {
    if (!vaultLogContainer) return;
    try {
      const logs = JSON.parse(localStorage.getItem('sweetheart_activity_log') || '[]');
      if (logs.length === 0) {
        vaultLogContainer.innerHTML = '<p class="vault-subtitle" style="text-align:center;">No activity recorded yet! Play games or interact on the page to record responses ❤️</p>';
        return;
      }

      vaultLogContainer.innerHTML = '';
      logs.forEach(item => {
        const div = document.createElement('div');
        div.className = 'vault-log-item';
        div.innerHTML = `
          <div class="vault-log-time">🕒 ${item.timestamp || 'Just now'}</div>
          <div class="vault-log-title">${item.title}</div>
          <div class="vault-log-detail">${item.detail}</div>
        `;
        vaultLogContainer.appendChild(div);
      });
    } catch (e) {
  ...
  if (vaultClose) {
    vaultClose.addEventListener('click', () => {
      activityVaultModal.classList.remove('active');
      activityVaultModal.setAttribute('aria-hidden', 'true');
    });
  }

  if (btnCopyLog) {
    btnCopyLog.addEventListener('click', () => {
      const logs = localStorage.getItem('sweetheart_activity_log') || '[]';
  ...
  if (btnClearLog) {
    btnClearLog.addEventListener('click', () => {
      if (confirm('Are you sure you want to clear the activity vault log?')) {
        localStorage.removeItem('sweetheart_activity_log');
        renderVaultLogs();
      }
    });
  }
